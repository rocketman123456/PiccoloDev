...
namespace Piccolo
{
    GPUPipelineBuilder::GPUPipelineBuilder(VkDevice device)
        : m_device(device)
    {
        reset();
    }
    ...
    VkPipeline GPUPipelineBuilder::buildGraphicsPipeline()
    {
        ...
        {
            default_color_blend_attachment.colorWriteMask =
    ...
    void GPUPipelineBuilder::cleanupShaders() { m_shaders.clear(); }
    ...
    GPUPipelineBuilderConfig GPUPipelineConfigFactory::createBasicTrianglePipeline()
    {
        ...
        config.shader_stages = {
            {   ShaderType::VertexShader, "asset/shader/glsl/shader_base.vert", "main"},
            {ShaderType::FragmanetShader, "asset/shader/glsl/shader_base.frag", "main"}
        };

        return config;
    }

    GPUPipelineBuilderConfig GPUPipelineConfigFactory::createAdvancedTrianglePipeline()
    {
        GPUPipelineBuilderConfig config;
        config.name        = "Advanced Triangle Pipeline";
        config.description = "Advanced pipeline for rendering triangles";
        ...
        config.shader_stages = {
            ...
            {ShaderType::FragmanetShader, "asset/shader/glsl/vert_buffer.frag", "main"}
        };
