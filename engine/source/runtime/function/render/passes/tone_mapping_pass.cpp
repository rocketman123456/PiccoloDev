...
// RHI types are referenced via headers included by render_pass.h; no direct includes needed here
...
namespace Piccolo
{
    void ToneMappingPass::initialize(const RenderPassInitInfo* init_info)
    {
        ...
        const ToneMappingPassInitInfo* tone_mapping_init_info = static_cast<const ToneMappingPassInitInfo*>(init_info);
        m_framebuffer.render_pass                            = tone_mapping_init_info->render_pass;

        setupDescriptorSetLayout();
        setupPipelines();
        setupDescriptorSet();
        updateAfterFramebufferRecreate(tone_mapping_init_info->input_attachment);
    }
