...
namespace Piccolo
{
    ...
    // {
    //     for (int i = 0; i < pose.bone_poses.size(); i++)
    //     {
    //         int bone_index = i;
    //         if (pose.reorder)
    //         {
    //             bone_index = pose.bone_indexs[i];
    //         }
    //         Bone& bone = m_bones[bone_index];
    //         bone.setOrientation(pose.bone_poses[i].m_rotation);
    //         bone.setScale(pose.bone_poses[i].m_scale);
    //         bone.setPosition(pose.bone_poses[i].m_position);
    //     }
    //     for (size_t i = 0; i < m_bone_count; i++)
    //     {
    //         m_bones[i].update();
    ...
    // {
    //     for (int i = 0; i < pose.bone_poses.size() && i < m_bone_count; i++)
    //     {
    //         int bone_index = i;
    //         if (pose.reorder)
    //         {
    ...
    //         }
    //         Bone& bone = m_bones[bone_index];
    //         bone.rotate(pose.bone_poses[i].m_rotation);
    //         bone.scale(pose.bone_poses[i].m_scale);
    //         bone.translate(pose.bone_poses[i].m_position);
    //     }
    //     for (size_t i = 0; i < m_bone_count; i++)
    //     {
    //         m_bones[i].update();
    ...
    // {
    //     pose.reorder = false;
    //     pose.bone_poses.resize(m_bone_count);
    //     for (int i = 0; i < m_bone_count; i++)
    //     {
    //         Bone& bone                    = m_bones[i];
    //         pose.bone_poses[i].m_rotation = bone.getOrientation();
    //         pose.bone_poses[i].m_scale    = bone.getScale();
    //         pose.bone_poses[i].m_position = bone.getPosition();
    //     }
    // }
} // namespace Piccolo