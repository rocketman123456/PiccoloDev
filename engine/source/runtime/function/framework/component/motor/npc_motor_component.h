...
namespace Piccolo
{
    REFLECTION_TYPE(NPCMotorComponent)
    CLASS(NPCMotorComponent : public Component, WhiteListFields,WhiteListMethods)
    {
        REFLECTION_BODY(NPCMotorComponent)
    public:
        NPCMotorComponent() = default;

        void postLoadResource(std::weak_ptr<GObject> parent_object) override;

        ~NPCMotorComponent() override;

        void tick(float delta_time) override;
        void tickNPCMotor(float delta_time);
    ...
    private:
        ...
        META(Enable)
        NPCMotorComponentRes m_motor_res;
