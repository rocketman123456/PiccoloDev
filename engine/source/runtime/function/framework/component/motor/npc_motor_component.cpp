#include "runtime/function/framework/component/motor/npc_motor_component.h"
...
namespace Piccolo
{
    void NPCMotorComponent::postLoadResource(std::weak_ptr<GObject> parent_object)
    {
        ...
        if (m_motor_res.m_controller_config.getTypeName() == "PhysicsControllerConfig")
        {
            ...
            m_controller                               = new NPCCharacterController(controller_config->m_capsule_shape);
        }
    ...
    void NPCMotorComponent::getOffStuckDead()
    {
        // TODO
        LOG_INFO("Some get off stuck dead logic");
    }

    NPCMotorComponent::~NPCMotorComponent()
    {
    ...
    void NPCMotorComponent::tick(float delta_time) { tickNPCMotor(delta_time); }

    void NPCMotorComponent::tickNPCMotor(float delta_time)
    {
    ...
    void NPCMotorComponent::calculatedDesiredHorizontalMoveSpeed(unsigned int command, float delta_time)
    {
    ...
    void NPCMotorComponent::calculatedDesiredVerticalMoveSpeed(unsigned int command, float delta_time)
    {
    ...
    void NPCMotorComponent::calculatedDesiredMoveDirection(unsigned int command, const Quaternion& object_rotation)
    {
        if (m_jump_state == JumpState::idle)
        {
    ...
    void NPCMotorComponent::calculateDesiredDisplacement(float delta_time)
    {
    ...
    void NPCMotorComponent::calculateTargetPosition(const Vector3&& current_position)
    {
