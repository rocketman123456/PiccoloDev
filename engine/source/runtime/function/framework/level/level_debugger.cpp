...
namespace Piccolo
{
    ...
    void LevelDebugger::drawBones(std::shared_ptr<GObject> object) const
    {
        ...
        for (int32_t bone_index = 0; bone_index < bones_count; bone_index++)
        {
            ...
            auto pos   = bones[bone_index]._getDerivedPosition();
            auto ori   = bones[bone_index]._getDerivedOrientation();
            auto scale = bones[bone_index]._getDerivedScale();

            Matrix4x4 bone_matrix = Transform(pos, ori, scale).getMatrix();
            ...
            auto  parent_pos   = parent_bone->_getDerivedPosition();
            auto  parent_ori   = parent_bone->_getDerivedOrientation();
            auto  parent_scale = parent_bone->_getDerivedScale();

            Matrix4x4 parent_bone_matrix = Transform(parent_pos, parent_ori, parent_scale).getMatrix();
