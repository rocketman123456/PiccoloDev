...
bool NativeFileSystem::isFolderExists(const std::filesystem::path& name) { return std::filesystem::exists(name) && std::filesystem::is_directory(name); }

bool NativeFileSystem::isFileExists(const std::filesystem::path& name) { return std::filesystem::exists(name) && std::filesystem::is_regular_file(name); }
...
std::shared_ptr<IBlob> RelativeFileSystem::readFile(const std::filesystem::path& name) { return underlyingFS_->readFile(basePath_ / name.relative_path()); }
